...
import { useAppContext } from '../context/AppContext'; // Ensure this is your custom context

export default function MealPlanTab() {
  const { mealPlan, setMealPlan } = useAppContext(); // Access global state for mealPlan and setMealPlan

  // Function to remove a meal from the meal plan
  const removeMeal = (id: number) => {
    setMealPlan((prev) => prev.filter((meal) => meal.id !== id)); // Update the global state
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        data={mealPlan} // Display mealPlan from context
